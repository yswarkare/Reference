...
const { getUserByEmailOrUsername, validateUsername, validateEmailId } = require("../Utils/Validations");
...
router.get("/user-profile", userAuth, async (req, res) => {
    await User.populate("ratings").populate("reviews");
...
router.patch("/update-user-username", userAuth, async (req, res) => {
    ...
    if ( usernameIsTaken === true){
        ...
        try {
            await User.findOneAndUpdate({_id: req.user._id}, {username: req.body.user.username});
            let updated = await User.findOne({_id: req.user_id});
...
router.patch("/update-user-email", userAuth, async (req, res) => {  
    ...
    if ( emailIsTaken === true){
        ...
        try {
            let updated = await User.findOneAndUpdate({_id: req.user._id}, {emailId: req.body.user.emailId});
...
router.patch("/update-user-password", userAuth, async (req, res) => {
    ...
    try {
        let updated = await User.findOneAndUpdate({_id: req.user}, {password, newHashedPassword})
